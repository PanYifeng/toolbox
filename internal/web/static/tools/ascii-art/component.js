...
// 注：含状态（img/invert/el）的函数必须嵌在组件闭包内，避免 ES module 顶层引用未定义变量。
...
// currentRamp 按 id 取字符集（找不到回退 classic）— 纯函数
function currentRamp(id) {
  return (RAMPS.find((r) => r.id === id) || RAMPS[0]).chars;
}

// toAscii 核心：按列数缩放采样，逐像素灰度映射字符，行数折半（高宽比 2:1）— 纯函数
...
// fallbackCopy 无 Clipboard API 时的降级复制 — 纯函数
...
export default function (el) {
  ...
  function onFile(e, $pv, done) {
    const file = e.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      const im = new Image();
      im.onload = () => { img = im; $pv.innerHTML = `<img src="${im.src}" alt="preview">`; done(); };
  ...
  function render($out) {
    ...
    const ramp = currentRamp(el.querySelector('#asc-ramp').value);
    const cols = Number(el.querySelector('#asc-width').value);
  ...
  function copyOut($out) {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(txt).then(() => hint()).catch(() => fallbackCopy(txt));
    } else {
      fallbackCopy(txt);
    }
  }
  ...
  function hint() {
    const $btn = el.querySelector('#asc-copy');
    if (!$btn) return;
