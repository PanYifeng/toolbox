export default {
  ...
  desc: 'MBTI 十六型人格测试，20 题强制选择快速得出你的四字母型态；免费看简版结果，完整版详细报告 ¥5 经站主确认后邮件送达。',
  guide: {
    zh: `## 功能

通过 20 道二选一题（每个维度 5 题）快速定位你的 MBTI 四字母型态（如 INTJ、ENFP），免费查看简版结果。完整版详细报告（型态深度解析 + 优势 / 盲点 / 职场 / 关系建议）为付费项，¥5 一次，经站主确认收款后邮件送达。
...
- **计分方式**：每个维度 5 题，你选的那一极计 1 票，多者胜出决定该维度字母
- **免费 vs 完整版**：免费版给四字母型态 + 一句简述；完整版给多段深度解析
- **完整版送达**：提交邮箱+交易号后，站主确认收款即把完整报告邮件发给你
- **数据安全**：作答仅在浏览器本地计分，不上传服务器（完整版报告文本随申请提交以邮件送达）`,
    en: `## Features

Answer 20 forced-choice questions (5 per dichotomy) to find your four-letter MBTI type (e.g. INTJ, ENFP) and view a free brief result. A detailed full report (type deep-dive + strengths / blind spots / career / relationship tips) is a paid item, ¥5 one-time, emailed after the site owner confirms payment.
...
- **Scoring**: 5 questions per dichotomy; the pole you pick earns one vote; the majority wins that letter
