// DISC 行为风格组件：16 道李克特题 → 四维度百分比 + 主导风格 → 免费简版解读，完整版 ¥5 经站主确认门邮件送达。
...
const ITEMS = Object.entries(data).filter(([k]) => /^[DISC]\d$/.test(k)).map(([k, v]) => ({ k, ...v }));
const AMOUNT = 5;
const DIMS = ['D', 'I', 'S', 'C'];

// snapshot { ratings:number[], pcts:object, primary:string, fullStage, claimId?, email? }
let snapshot = null;
...
export function render(el) {
  ...
  el.innerHTML = `
    <div class="rel-intro">
      <p class="muted">${t('disc.intro')}</p>
      <p class="muted">${t('disc.qtip')}</p>
      <button class="btn" id="disc-start">${t('disc.start')}</button>
    </div>`;
  el.querySelector('#disc-start').onclick = () => startTest(el, lang);
}

// startTest 渲染 16 题 Likert 表单
function startTest(el, lang) {
  const ratings = new Array(ITEMS.length).fill(0);
  el.innerHTML = `
    <div class="quiz-bar">
      <span class="disc-progress">0 / ${ITEMS.length}</span>
  ...
  ITEMS.forEach((item, i) => {
    ...
    scale.forEach((lbl, j) => {
      ...
      b.querySelector('input').onchange = () => {
        ...
        el.querySelectorAll('.disc-progress').forEach((p) => { p.textContent = `${done} / ${ITEMS.length}`; });
      };
      $sc.appendChild(b);
    });
    $list.appendChild(card);
  });
  el.querySelectorAll('.js-disc-submit').forEach((b) => { b.onclick = () => onSubmit(el, ratings, lang); });
}
...
function onSubmit(el, ratings, lang) {
  ...
  snapshot = { ratings: [...ratings], pcts, primary, fullStage: 'pending' };
...
// renderResult 免费画像 + 主导风格 + 完整版入口 + 重新测试
