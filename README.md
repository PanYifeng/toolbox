...
> 在线 demo：https://app.allin1box.dpdns.org（44 个工具 · 纯前端零成本 · 可安装到桌面）
...
| 类别 | 工具 |
|------|------|
| 编码 | JSON 格式化、Base64、URL、哈希计算(SHA)、JWT 解码、HTML 实体 |
| 时间 | 时间戳转换、时间长度换算 |
| 文本 | 正则测试、大小写、文本统计、Markdown 预览、文本差异、Slug、文本行处理 |
| 生成 | UUID、密码、Lorem ipsum |
| 数学 | 进制转换 |
| 设计 | 颜色转换、像素画板、ASCII 艺术生成器 |
| 网络 | URL 解析 |
| 视频 | 视频转码（ffmpeg）、视频截断（ffmpeg 流复制，无损不重编码） |
| 音频 | 音频转换（ffmpeg：mp3/wav/flac/ogg/m4a） |
| 文档 | 文档转换（docx↔pdf 等） |
| 宗教文化 | 佛教文化、基督文化、伊斯兰文化（考试 + 错题解析 + 结业纪念卡） |
| 知识 · 趣味 | 趣味知识问答（及格发卡 / 付费错题解析）、MBTI / 大五人格 / DISC（免费简版 + ¥5 完整版报告） |
| 游戏 | 2048、贪吃蛇、井字棋、蜘蛛纸牌、扫雷、俄罗斯方块、Flappy Bird、Wordle、合成水果（均纯前端，计分 + 通关纪念卡 + 排行榜） |
| 纪念卡 | 纪念卡验真（防伪码复算校验） |
...
非 API 计费、面向 C 端用户的轻变现入口，均纯前端 + 邮件核销，无需支付网关：

- **通关纪念卡**：游戏通关 / 考试及格生成 PNG 纪念卡（防伪码由 姓名+主题+分数+时间 SHA-256 派生，前端可复算验真）。破纪录金版卡由服务端 HMAC 签发，经站主邮件确认收款后下发下载。
- **排行榜**：每游戏前十名常驻；破纪录自动入榜并触发金版卡核验。
- **付费完整版 / 错题解析**：人格测试完整版报告（¥5）、知识/宗教考试错题解析（0.2 元/题）。访客提交邮箱 + 支付交易号，站主收到邮件点确认链接（HMAC 签名）后内容原样邮件送达访客。后端通用 `paidreport` / `errata` 模块复用同一确认门模式。
